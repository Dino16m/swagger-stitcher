...
pub fn stitch(options: StitcherOptions) {
  let mut base_spec = get_base_spec(options.base.as_str());
  let exclusion_list: Vec<&str> = options.exclude.iter().map(|val| val.as_str()).collect();
  let other_specs: Vec<Hash> = get_yaml_specs(options.directory.as_str(), exclusion_list);
  base_spec = merge_specs(base_spec, other_specs);
  let mut out_str = String::new();
  let mut emitter = YamlEmitter::new(&mut out_str);
  emitter.dump(&Yaml::Hash(base_spec)).unwrap();
  write_output(out_str, options.output.as_str());
  println!("YAML docs written to {:?} ", options.output);
}
